...
public static class ExtensionHelpers {
    ...
    public static string? GetSummaryText(this GeneratorAttributeSyntaxContext context) {
        ...
        var summary = args
            ...
            .Where(n => string.Equals(nameof(Partialor.PartialAttribute.Summary), n.NameEquals?.Name.Identifier.ValueText, StringComparison.Ordinal))
    ...
    public static string? GetPartialClassName(this GeneratorAttributeSyntaxContext context) {
        ...
        var partialClassName = args
            .Where(n => n.Expression.IsKind(SyntaxKind.StringLiteralExpression))
            .Where(n => string.Equals(nameof(Partialor.PartialAttribute.PartialClassName), n.NameEquals?.Name.Identifier.ValueText, StringComparison.Ordinal))
            .Select(n => n.Expression)
            .OfType<LiteralExpressionSyntax>()
            .Select(n => n.Token.ValueText)
            .SingleOrDefault();
    ...
    /// <param name="listEntityToKeep">The entity to keep</param>
    /// <returns>The root node with only the entity to keep as a member node</returns>
    public static SyntaxNode FilterOutEntitiesExcept(this SyntaxNode root, SyntaxNode[] listEntityToKeep) {
        var typeDefinitions = root.DescendantNodes().OfType<BaseTypeDeclarationSyntax>();

        foreach (var t in typeDefinitions) {
            bool keep = false;
            foreach (var entityToKeep in listEntityToKeep) {
            ...
            if (keep) {
                ...
                return FilterOutEntitiesExcept(newNode, listEntityToKeep);
            }
        }
    ...
    /// <summary>
    /// ExtensionHelpers.WrapPartialValue
    /// </summary>
    /// <param name="type"></param>
    /// <returns></returns>
    public static NameSyntax WrapPartialValue(TypeSyntax type) {
        var globalQualifiedName =
            SyntaxFactory.AliasQualifiedName(
                SyntaxFactory.IdentifierName(SyntaxFactory.Token(SyntaxKind.GlobalKeyword)),
                SyntaxFactory.IdentifierName("Partialor"))
            .WithColonColonToken(SyntaxFactory.Token(SyntaxKind.ColonColonToken));

        var fullName = SyntaxFactory.QualifiedName(
            globalQualifiedName,
            SyntaxFactory.GenericName(
                SyntaxFactory.Identifier("PartialValue"),
    ...
    /// <summary>
    /// Partialor.ExtensionHelpers.ToTypeSyntax
    /// </summary>
    /// <param name="symbol"></param>
    /// <returns></returns>
    /// <exception cref="ArgumentNullException"></exception>
    public static TypeSyntax ToTypeSyntax(INamedTypeSymbol symbol) {
        if (symbol == null)
        ...
        // Roslyn uses "global::" prefix for fully qualified names
    ...
    public static NamespaceDeclarationSyntax? GetNamespace(TypeDeclarationSyntax typeDecl) {
        ...
        while (current != null) {
            switch (current) {
                case NamespaceDeclarationSyntax ns: {
                        var idn = SyntaxFactory.IdentifierName(ns.Name.ToString());
                        list.Insert(0, idn);
                        break;
                    }

                case FileScopedNamespaceDeclarationSyntax fsNs: {
                        var idn = SyntaxFactory.IdentifierName(fsNs.Name.ToString());
                        list.Insert(0, idn);
                        break;
                    }
                default:
        ...
        if (0 == list.Count) {
            ...
            return SyntaxFactory.NamespaceDeclaration(list[0]);
        } else {
            NameSyntax result = list[0];
            list.RemoveAt(0);
            while (0 <= list.Count) {
                var nameNext = list[0];
                list.RemoveAt(0);
                result = SyntaxFactory.QualifiedName(result, nameNext);
            }
            return SyntaxFactory.NamespaceDeclaration(result);
        }
    }
